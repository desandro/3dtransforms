...
Take for instance the Weather App on the iPhone. The application uses two views, a details view and an options view. Switching between these two views is done with a 3D flip transition. This affords the user that interface has two, and only two views, as they can only exist on either side of the same plane.

!img/weather-app-transition.jpg(Weather App transition)!

Also consider image cycle interfaces. When you're at the last slide, what cues tip-off the user that the advancing will re-start the cycle at the first? A better paradigm can be used with a 3D transform, where the slides are places side by side one another _in a circle in 3D space_. In that arrangement, the last slide logically does come before the first.

Not only can we use 3D transforms to create more visually impressive applications, but we can also use them to solve dilemmas and make our applications more intuitive. 

h2. Current Support Environment

"The CSS3 spec for 3D transforms":http://www.w3.org/TR/css3-3d-transforms/ has been out in the wild for a year now, but currently, they are only supported by Safari -- which includes Safari on the desktop and Mobile Safari on iOS devices). Support on other browsers varies. 
