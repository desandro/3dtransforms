...
Ladies and gentlemen, it is the second decade of the second millenium and we are still kicking around the same 2D interface we got two decades ago. Sure, Apple debuted some 10.7 apps that added a couple more 3D flourishes, and Microsoft has had that "Flip 3D":http://windows.microsoft.com/en-US/windows-vista/Using-Windows-Flip-3D for a while. But c'mon, 2011 is in a couple weeks. Where is our 3D virtual reality? By now, we should be zipping around the "Metaverse in super-sonic motorbikes":http://en.wikipedia.org/wiki/Snow_Crash.
...
bc. #box figure:nth-child(1) { -webkit-transform: none; }
#box figure:nth-child(2) { -webkit-transform: rotateX( 180deg ); }
#box figure:nth-child(3) { -webkit-transform: rotateY( 90deg ); }
#box figure:nth-child(4) { -webkit-transform: rotateY( -90deg ); }
#box figure:nth-child(5) { -webkit-transform: rotateX( 90deg ); }
#box figure:nth-child(6) { -webkit-transform: rotateX( -90deg ); }
...
bc. #box figure:nth-child(1) { -webkit-transform: translateZ( 100px ); }
#box figure:nth-child(2) { -webkit-transform: rotateX( 180deg ) translateZ( 100px ); }
#box figure:nth-child(3) { -webkit-transform: rotateY( 90deg ) translateZ( 100px ); }
#box figure:nth-child(4) { -webkit-transform: rotateY( -90deg ) translateZ( 100px ); }
#box figure:nth-child(5) { -webkit-transform: rotateX( 90deg ) translateZ( 100px ); }
#box figure:nth-child(6) { -webkit-transform: rotateX( -90deg ) translateZ( 100px ); }
...
bc. #box.show-1 { -webkit-transform: translateZ(-100px); }
#box.show-2 { -webkit-transform: translateZ(-100px) rotateX( -180deg); }
#box.show-3 { -webkit-transform: translateZ(-100px) rotateY(-90deg); }
#box.show-4 { -webkit-transform: translateZ(-100px) rotateY(90deg); }
#box.show-5 { -webkit-transform: translateZ(-100px) rotateX(-90deg); }    
#box.show-6 { -webkit-transform: translateZ(-100px) rotateX( 90deg); }
...
/* front and back */
#box figure:nth-child(1),
#box figure:nth-child(2) {
  width: 296px;
  height: 196px;
}
/* left and right sides */
#box figure:nth-child(3),
#box figure:nth-child(4) {
  width: 96px;
  height: 196px;
  left: 100px;
}
/* top and bottom */
#box figure:nth-child(5),
#box figure:nth-child(6) {
  width: 296px;
  height: 96px;
  top: 50px;
  line-height: 96px;
...
The smaller faces (left, right, top and bottom) need to be positioned in the center of the container. So the thinner faces, left and right, get positioned @left 100px@, The stouter top and bottom faces get positioned @top: 50px@.

Just like the cube, the faces need to be rotated and translated out. The rotate values all can remain the same, but for this rectangular prism, the translate values do differ. The front and back faces each are shifted out @50px@ since the box is 100px deep. Left and right faces translate is @150px@ for 300px width. Top and bottom panels go @100px@ for the 200px height.

bc. #box figure:nth-child(1) { -webkit-transform: translateZ( 50px ); }
#box figure:nth-child(2) { -webkit-transform: rotateX( 180deg ) translateZ(  50px ); }
#box figure:nth-child(3) { -webkit-transform: rotateY(  90deg ) translateZ( 150px ); }
#box figure:nth-child(4) { -webkit-transform: rotateY( -90deg ) translateZ( 150px ); }
#box figure:nth-child(5) { -webkit-transform: rotateX(  90deg ) translateZ( 100px ); }
#box figure:nth-child(6) { -webkit-transform: rotateX( -90deg ) translateZ( 100px ); }

Again, to expose a face, the box needs to have a style to reverse its transform.

bc. #box.show-1 { -webkit-transform: translateZ(  -50px ); }
#box.show-2 { -webkit-transform: translateZ(  -50px ) rotateX( -180deg ); }
#box.show-3 { -webkit-transform: translateZ( -150px ) rotateY(  -90deg ); }
#box.show-4 { -webkit-transform: translateZ( -150px ) rotateY(   90deg ); }
#box.show-5 { -webkit-transform: translateZ( -100px ) rotateX(  -90deg ); }
#box.show-6 { -webkit-transform: translateZ( -100px ) rotateX(   90deg ); }
